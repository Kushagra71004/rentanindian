...
export default function Auth() {
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(158deg,#0C52A0 0%,#1568BF 25%,#2E8CE8 55%,#EBF5FF 100%)', padding: '80px 20px' }}>
      <div style={{ width: '100%', maxWidth: 420, background: '#fff', borderRadius: 'var(--rx)', padding: '48px 40px', boxShadow: '0 24px 72px rgba(0,0,0,.18)', animation: 'fadeUp .45s ease forwards' }}>
        {/* Logo */}
        <div style={{ textAlign: 'center', marginBottom: 8 }}>
          <div style={{ fontFamily: 'var(--fd)', fontSize: 20, fontWeight: 800, color: 'var(--ink)', letterSpacing: '-.02em' }}>
            rentanindian<span style={{ color: 'var(--sky)' }}>.ai</span>
          </div>
          <div style={{ fontSize: 12.5, color: 'var(--ink4)', marginTop: 4, fontStyle: 'italic' }}>
        ...
        {mode === 'register' && (
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              {[{ val: 'user', label: '👤 User', desc: 'Post & accept tasks' }, { val: 'admin', label: '🛡 Admin', desc: 'Full platform control' }].map(r => (
                <div key={r.val} onClick={() => setForm({ ...form, role: r.val })} style={{
                  border: `1.5px solid ${form.role === r.val ? 'var(--sky)' : 'var(--border)'}`,
                  borderRadius: 10, padding: '12px 14px', cursor: 'pointer', transition: 'all .18s',
                  background: form.role === r.val ? 'var(--sky-pale)' : '#fff',
                }}>
                  <div style={{ fontWeight: 700, fontSize: 13.5, color: form.role === r.val ? 'var(--sky)' : 'var(--ink)' }}>{r.label}</div>
                  <div style={{ fontSize: 11.5, color: 'var(--ink4)', marginTop: 3 }}>{r.desc}</div>
                </div>
              ))}
            </div>
          </div>
