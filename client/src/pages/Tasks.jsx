        ...
        <div style={{ maxWidth: 1120, margin: '0 auto' }}>
          <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap', alignItems: 'center', marginBottom: 32 }}>
            <div style={{ position: 'relative', flex: 1, minWidth: 220 }}>
              ...
              <input className="form-input" type="text" placeholder="Search tasks…" value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 40 }} />
            </div>
          ...
          {loading ? (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: 20 }}>
              {filtered.map(task => (
                <TaskCard key={task._id} task={task} onAccepted={fetchTasks} onDeleted={fetchTasks} />
              ))}
            </div>
          )}
        </div>
